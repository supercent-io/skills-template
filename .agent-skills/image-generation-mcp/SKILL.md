...
A skill for AI image generation via MCP. Uses Gemini models or compatible services to generate high-quality images for marketing, UI, and presentations.
...
- **Brand consistency**: Style-guide-based image generation
...
- API key reference (stored as environment variable)
- Output directory
...
**Style**: [Style - minimal, illustration, photographic, 3D, etc.]
**Lighting**: [Lighting - natural light, studio, golden hour, etc.]
...
|-------|----------|-----------------|
| `gemini-3-pro-image` | High quality | Complex compositions, fine detail |
...
- Model, aspect ratio, and usage notes
...
1. **Specify ratio early**: Prevent unintended cropping
2. **Use style anchors**: Maintain a consistent aesthetic style
3. **Iterate with constraints**: Change only one variable at a time
4. **Track prompts**: Ensure reproducibility
5. **Batch similar requests**: Generate consistent style sets
...
- **Ignoring size constraints**: Check size requirements for the target channel
...
**Cause**: Ratio not specified or unsupported ratio used
**Solution**: Provide the exact ratio and regenerate

### Issue: Brand mismatch
**Cause**: Color codes not specified
**Solution**: Specify brand colors using HEX codes
...
- **Round 1 (Orchestrator)**: Prompt completeness, ratio consistency
- **Round 2 (Analyst)**: Style consistency, brand alignment
- **Round 3 (Executor)**: Output filenames, delivery checklist verification

### Agent Roles

| Agent | Role |
|-------|------|
| Claude | Prompt composition, quality verification |
| Gemini | Image generation execution |

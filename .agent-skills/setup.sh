...
# Generated by setup.sh v3.3
# Note: This script is designed to be COMPLETELY silent when sourced

# Suppress ALL output (stdout + stderr) during path detection
{
...
# Load helper functions silently (suppress both stdout and stderr)
if [ -f "$AGENT_SKILLS_PATH/mcp-skill-loader.sh" ]; then
    source "$AGENT_SKILLS_PATH/mcp-skill-loader.sh"
fi
} > /dev/null 2>&1

# Skill query aliases (aliases don't produce output)
...
# Export functions - suppress ALL output (zsh outputs function definitions to stdout)
{
    export -f gemini-skill codex-skill mcp-status
} > /dev/null 2>&1 || true

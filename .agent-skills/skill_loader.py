        ...
        while i < len(lines):
            ...
            if array_match:
                ...
                while i < len(lines):
                    ...
                    if re.match(r'^\s*([^\[]+)\[([^\]]*)\]\{([^}]*)\}:\s*$', row_line) or re.match(r'^\s*([^:\[]+):\s*(.*)$', row_line):
                    ...
                    # Parse CSV row
                    try:
                        reader = csv.reader(io.StringIO(row_line), delimiter=',', quotechar='"')
                    ...
                    if re.match(r'^\s*([^\[]+)\[([^\]]*)\]\{([^}]*)\}:\s*$', row_line) or (re.match(r'^\s*([^:\[]+):\s*(.*)$', row_line) and ',' not in row_line):
                        break
...
class SkillLoader:
    ...
    def _parse_toon_skill(self, skill_path: Path) -> Optional[Dict]:
        try:
            ...
            return {
                ...
                'allowed_tools': data.get('allowed_tools', []), # Expecting list if supported, or parsing string
    ...
    def validate_skill(self, skill: Dict) -> List[str]:
        ...
        if not re.match(r'^[a-z0-9]+(-[a-z0-9]+)*$', name):
            problems.append(f"Invalid name '{name}': Must be lowercase, hyphen-separated, no consecutive hyphens, start/end with alphanumeric.")
        ...
        if dir_name != name:
            problems.append(f"Directory name '{dir_name}' does not match skill name '{name}'.")
...
def main():
    ...
    parser.add_argument('--dir', default='.agent-skills', help='Path to skills directory')
    ...
    prompt_parser.add_argument('--format', choices=['markdown', 'xml', 'json', 'toon'], default='markdown', help='Output format')
    prompt_parser.add_argument('--output', help='Output file')
    
    # New command: validate
    validate_parser = subparsers.add_parser('validate', help='Validate skills against strict standards')
    validate_parser.add_argument('name', nargs='?', help='Specific skill name to validate')
